...
/**
 * @title SimpleDocumentSharing
 * @dev Simplified smart contract for managing document sharing permissions on IPFS
 * @notice This contract manages access control for documents stored on IPFS without external dependencies
 */
contract SimpleDocumentSharing {
     ...
     * @return documentOwner Owner address
    ...
    function getDocument(bytes32 documentId) 
        ...
        returns (string memory ipfsHash, address documentOwner, uint256 createdAt, string memory metadata) 
    {
        ...
        onlyDocumentOwner(documentId) 
    {
        ...
        for (uint256 i = 0; i < accessors.length; i++) {
            accessList[documentId][accessors[i]] = false;
        }
    ...
    /**
     * @dev Emergency pause function (only contract owner)
     */
    ...
    /**
     * @dev Emergency unpause function (only contract owner)
     */
